...
Vavn_aux, navn_aux, mavn_aux =np.zeros((avn+1,avn+1)), np.zeros((avn+1,avn+1)), np.zeros((avn+1,avn+1))
...
Vavn = np.full((avn+1,avn+1),-10.9506)
...
navn = np.full((avn+1,avn+1),0.1702)
...
mavn = np.full((avn+1,avn+1),0.1702)
...
#Parameters and other permanent text to the main figure, . we are using universal coordinates (in the interval [0,1])


lista_ecgI = list()
lista_ecgII = list()
lista_ecgIII = list()
# The figure will have 3 subplots, their positions will be chosen automatically

#This subplot uses a color code

# The following lines of code initializes de figure

ax1 = fig.add_subplot(spec[0])
#ax1 = fig.add_subplot()

im1 = ax1.imshow(V1, cmap=plt.cm.RdBu_r,interpolation='gaussian',vmin=-12,vmax=120,animated=True,origin="lower")


#line1, = ax1.plot((M/4,M/4), (M/4,3*M/4), linewidth=1, color='grey')
#line2, = ax1.plot((M/4,3*M/4), (3*M/4,3*M/4), linewidth=1, color='grey')
#line3, = ax1.plot((3*M/4,3*M/4), (3*M/4,M/4), linewidth=1, color='grey')
#line4, = ax1.plot((M/4,3*M/4), (M/4,M/4), linewidth=1, color='grey')
ax1.set_xlabel("x")
...
im2 = ax2.imshow(V1, cmap=plt.cm.RdBu_r,interpolation='gaussian',vmin=-12,vmax=120,animated=True,origin="lower")
ax2.axis('off')
...
def update(t):
	...
	for i in range(Tint):
		...
		Vbundle_aux[0] = V_func_prk(Vbundle[0],nbundle[0],m1[0],0,0) + 1/6*(Vavn[int(avn/2),avn]-Vbundle[0])
		...
		#AVN COMPLEX
		#center cell of AVN
		Vavn_aux[int(avn/2),0]= V_func_fast(Vavn[int(avn/2),0],navn[int(avn/2),0],mavn[int(avn/2),0], const(I),I) + 1/6*(V5[int(3*ra/4),int(3*ra/4)]+Vavn[int(avn/2),1]-2*Vavn[int(avn/2),0])
		navn_aux[int(avn/2),0]= n_func_ventricle(Vavn[int(avn/2),0],navn[int(avn/2),0],mavn[int(avn/2),0])
		mavn_aux[int(avn/2),0]= m_func(Vavn[int(avn/2),0],navn[int(avn/2),0],mavn[int(avn/2),0])
        #top branch
		Vavn_aux[1:avn,1]= V_func_fast(Vavn[1:avn,1],navn[1:avn,1],mavn[1:avn,1], const(I),I) + 1/6*(Vavn[2:avn+1,1]+Vavn[0:avn-1,1]-2*Vavn[1:avn,1])
		navn_aux[1:avn,1]= n_func_ventricle(Vavn[1:avn,1],navn[1:avn,1],mavn[1:avn,1])
		mavn_aux[1:avn,1]= m_func(Vavn[1:avn,1],navn[1:avn,1],mavn[1:avn,1])
        #middle cell top branch
		Vavn_aux[int(avn/2),1]= V_func_fast(Vavn[int(avn/2),1],navn[int(avn/2),1],mavn[int(avn/2),1], const(I),I) + 1/6*(Vavn[int(avn/2)-1,1]+Vavn[int(avn/2)+1,1]+Vavn[int(avn/2),0]-3*Vavn[int(avn/2),1])
		navn_aux[int(avn/2),1]= n_func_ventricle(Vavn[int(avn/2),1],navn[int(avn/2),1],mavn[int(avn/2),1])
		mavn_aux[int(avn/2),1]= m_func(Vavn[int(avn/2),1],navn[int(avn/2),1],mavn[int(avn/2),1])
        #top corners
		Vavn_aux[0,1]= V_func_fast(Vavn[0,1],navn[0,1],mavn[0,1], const(I),I) + 1/6*(Vavn[1,1]+Vavn[0,2]-2*Vavn[0,1])
		navn_aux[0,1]= n_func_ventricle(Vavn[0,1],navn[0,1],mavn[0,1])
		mavn_aux[0,1]= m_func(Vavn[0,1],navn[0,1],mavn[0,1])
		
		Vavn_aux[int(avn),1]= V_func_fast(Vavn[int(avn),1],navn[int(avn),1],mavn[int(avn),1], const(I),I) + 1/6*(Vavn[int(avn)-1,1]+Vavn[int(avn),2]-2*Vavn[int(avn),1])           
		navn_aux[int(avn),1]= n_func_ventricle(Vavn[int(avn),1],navn[int(avn),1],mavn[int(avn),1])
		mavn_aux[int(avn),1]= m_func(Vavn[int(avn),1],navn[int(avn),1],mavn[int(avn),1])
		
		
        #left branch
		Vavn_aux[0,2:avn-1]= V_func_fast(Vavn[0,2:avn-1],navn[0,2:avn-1],mavn[0,2:avn-1], const(I),I) + 1/6*(Vavn[0,3:avn]+Vavn[0,1:avn-2]-2*Vavn[0,2:avn-1])
		navn_aux[0,2:avn-1]= n_func_ventricle(Vavn[0,2:avn-1],navn[0,2:avn-1],mavn[0,2:avn-1])
		mavn_aux[0,2:avn-1]= m_func(Vavn[0,2:avn-1],navn[0,2:avn-1],mavn[0,2:avn-1])

		Vavn_aux[avn,2:avn-1]= V_func_fast(Vavn[avn,2:avn-1],navn[avn,2:avn-1],mavn[avn,2:avn-1], const(I),I) + 1/150*(Vavn[avn,3:avn]+Vavn[avn,1:avn-2]-2*Vavn[avn,2:avn-1])
		navn_aux[avn,2:avn-1]= n_func_ventricle(Vavn[avn,2:avn-1],navn[avn,2:avn-1],mavn[avn,2:avn-1])
		mavn_aux[avn,2:avn-1]= m_func(Vavn[avn,2:avn-1],navn[avn,2:avn-1],mavn[avn,2:avn-1])
        #bottom branch
		Vavn_aux[1:avn,avn-1]= V_func_fast(Vavn[1:avn,avn-1],navn[1:avn,avn-1],mavn[1:avn,avn-1], const(I),I) + 1/6*(Vavn[2:avn+1,avn-1]+Vavn[0:avn-1,avn-1]-2*Vavn[1:avn,avn-1])
		navn_aux[1:avn,avn-1]= n_func_ventricle(Vavn[1:avn,avn-1],navn[1:avn,avn-1],mavn[1:avn,avn-1])
		mavn_aux[1:avn,avn-1]= m_func(Vavn[1:avn,avn-1],navn[1:avn,avn-1],mavn[1:avn,avn-1])
        #bottom corners

		Vavn_aux[0,avn-1]= V_func_fast(Vavn[0,avn-1],navn[0,avn-1],mavn[0,avn-1], const(I),I) + 1/6*(Vavn[1,avn-1]+Vavn[0,avn-2]-2*Vavn[0,avn-1])
		navn_aux[0,avn-1]= n_func_ventricle(Vavn[0,avn-1],navn[0,avn-1],mavn[0,avn-1])
		mavn_aux[0,avn-1]= m_func(Vavn[0,avn-1],navn[0,avn-1],mavn[0,avn-1])
		
		Vavn_aux[int(avn),avn-1]= V_func_fast(Vavn[int(avn),avn-1],navn[int(avn),avn-1],mavn[int(avn),avn-1], const(I),I) + 1/6*(Vavn[int(avn)-1,avn-1]+Vavn[int(avn),avn-2]-2*Vavn[int(avn),avn-1])
		navn_aux[int(avn),avn-1]= n_func_ventricle(Vavn[int(avn),avn-1],navn[int(avn),avn-1],mavn[int(avn),avn-1])
		mavn_aux[int(avn),avn-1]= m_func(Vavn[int(avn),avn-1],navn[int(avn),avn-1],mavn[int(avn),avn-1])

		Vavn_aux[int(avn/2),avn]= V_func_fast(Vavn[int(avn/2),avn],navn[int(avn/2),avn],mavn[int(avn/2),avn], const(I),I) + 1/6*(Vavn[int(avn/2),avn-1]-Vavn[int(avn/2),avn])
		navn_aux[int(avn/2),avn]= n_func_ventricle(Vavn[int(avn/2),avn],navn[int(avn/2),avn],mavn[int(avn/2),avn])
		mavn_aux[int(avn/2),avn]= m_func(Vavn[int(avn/2),avn],navn[int(avn/2),avn],mavn[int(avn/2),avn])
		#UNIDIRECTIONAL BLOCK

		Vavn_aux[int(avn/4),1]= V_func_fast(Vavn[int(avn/4),1],navn[int(avn/4),1],mavn[int(avn/4),1], const(I),I) + 1/6*(Vavn[int(avn/4)-1,1]-Vavn[int(avn/4),1])
		navn_aux[int(avn/4),1]= n_func_ventricle(Vavn[int(avn/4),1],navn[int(avn/4),1],mavn[int(avn/4),1])
		mavn_aux[int(avn/4),1]= m_func(Vavn[int(avn/4),1],navn[int(avn/4),1],mavn[int(avn/4),1])
		...
		Vavn[:,:], navn[:,:], mavn[:,:] = Vavn_aux, navn_aux, mavn_aux
...
def update_fig(i):   #update_fig(i)
	...
	im2.set_array(rotate(Vavn, angle=90))
	return  
...
with open("../outputs/ecg_data_avnrt.csv", 'w', newline='') as file: #Change output file name
